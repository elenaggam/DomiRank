...
time_file = open(f"{N}_{avgN}_time.txt", "w")
...
for i in range(len(network_functions)):
    ...
    base = f"{network_titles[i]}/{N}_{avgN}_{sampling}/"
    if not os.path.exists(base):
        os.makedirs(base)

    figure = plt.figure()

    f.average_domi_attack(N, network_functions[i], network_args[i], base = base, avgN = avgN, sampling = sampling)
    lcc, links = np.loadtxt(f"{base}Domirank_averaged_lcc_links.txt", unpack = True)
    ...
    with open(f"{base}sigma.txt", "r") as f_sigma:
        data = f_sigma.readlines()
        sigma = float(data[-1].split()[0])
        eig_sigma = float(data[0].split()[1])
    plt.title(f"{network_titles[i]} σ={sigma*eig_sigma:.2f}/λ", fontsize=14)
